...
const Le_personel_de_Lambassade=() =>  {
  const dataOfEmployee =[{id:1, name:"M. Aimé Yves BADO",occupation:" Ministre conseiller",image:"AimeYvesBADO.jpg"},
      ...
      {id:5,name:"Ms Somadula Sangeeta",occupation:"Secrétaire ",image:"Somadula_Sangeeta.png"},
...
return (
  <div className={styles.components}>
    <Text style={{ fontWeight: "bold", fontSize: "55px", textAlign: "center", marginBottom: "18px",wordBreak:"break-word" }}>Le Personnel de l'ambassade</Text>
    <div className={styles.stylepersonneldelambassade} >
      {dataOfEmployee.map(item => <div key={item.id}  style={{ backgroundColor: "#999933", display: "flex", flexDirection: "column", borderRadius: "10px", width:"250px",height:"300px" }}  ><div style={{ width: "200px", height: "50%", position: "relative", display: "flex", alignSelf: "center", marginTop: "15px" }}><Image style={{ borderRadius: "50%", display: "flex", textAlign: "center", width: "100%", height: "100%" }} width={1000} height={1000} src={`/images/employee_photo/${item.image}`} alt='' /></div><Text style={{ textAlign: "center",fontWeight:"bold" }}>{item.name}</Text><Text style={{ textAlign: "center" }}>{item.occupation}</Text></div>)}
    </div>
  </div>
)
}
...
const About_The_Embassy=()=>{
  return (
    <div className={styles.components}>
      <Text style={{textAlign:"center",fontWeight:"bold",fontSize:"55px",marginBottom:"18px",wordBreak:"break-word"}}>L'ambassade</Text>
      ...
      <Tabs defaultValue="L'ambassadeur" orientation="vertical">
        <Tabs.List style={{backgroundColor:"#ffec99",width:"30%", height:"50%"}}>
          ...
          <Tabs.Tab value="About_The_Embassy"  style={{width:"100%"}}>
            <Text> L'ambassade</Text> 
          </Tabs.Tab>
          <Tabs.Tab value="Reseau_consulaire" style={{width:"100%",wordBreak:"break-word"}} >
          <Text style={{whiteSpace:"wrap"}}>Le Reseau consulaire</Text>
          </Tabs.Tab>
